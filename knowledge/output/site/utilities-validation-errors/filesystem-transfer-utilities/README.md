# Filesystem & Transfer Utilities

Helpers for directory creation, transfers, archive creation, linking and ownership/permission checks used by import and storage workflows.
...
- **Validate filesystem path ownership by current user** — Describes a utility function assert_same_owner(path) that verifies a filesystem path is owned by the same user. It is intended to be used in CLI utilities or scripts to enforce ownership constraints before performing operations on files or directories.
  [Source](https://github.com/papaemmelab/isabl_cli/blob/main/isabl_cli/utils.py)
- **Verify current user is configured admin** — Checks whether the running user matches system_settings.ADMIN_USER and raises a PermissionError otherwise. An optional message parameter can customize the error text shown to the user. This safeguard is used by CLI commands that require administrative privileges within the Isabl environment.
  [Source](https://github.com/papaemmelab/isabl_cli/blob/main/isabl_cli/utils.py)
